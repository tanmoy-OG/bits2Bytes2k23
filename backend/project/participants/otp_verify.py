...
from key.keys import otp_virify_secret_key,admin_secret_key,coordinetor_secret_key,participants_secret_key,authorization_token_key

class otp_verify(Resource):
    def post(self):
        if 'signup' in request.headers:
        ...
        if 'signup' in verify_type:
            ...
            if user and user.otp==data['otp']:
                ...
                if user.roll is not None:
                    userss=Participants(roll=user.roll)
