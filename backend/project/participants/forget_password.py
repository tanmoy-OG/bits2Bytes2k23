...
class forget_password(Resource):
    def post(self):
        ...
        if user and len(data)==2 and data['email'] and data['password']:
            ...
            check=Temp_otp.query.filter_by(email=data['email']).first()
            if check:
                db.session.delete(check)
            us=Temp_otp(email=data['email'],otp=otp,password=generate_password_hash(data['password']))
            db.session.add(us)
            db.session.commit()
            msg="hi,{user.fname} this {otp} is for forget password"
            send_otp('forget password',data['email'],msg)
            response=make_response(jsonify({"successful":"otp sent successful"}))
            payload={"email":f"{data['email']}","forget":True}
            token=jwt.encode(payload,otp_virify_secret_key,algorithm='HS256')
            response.headers.set('forget',token)
            return response
        else:
